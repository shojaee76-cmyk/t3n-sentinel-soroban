...
/// Deploy the SAC contract with a fresh stellar asset contract as the asset.
/// Returns (env, contract_id, asset_addr, authority, worker).
fn setup() -> (Env, Address, Address, Address, Address) {
    let env = Env::default();
    let authority = Address::generate(&env);
    let worker = Address::generate(&env);
    let contract_id = env.register(SentinelSac, ());
    let client = SentinelSacClient::new(&env, &contract_id);

    // Register a stellar asset contract (SAC) to act as USDC-on-Stellar.
    let sac = env.register_stellar_asset_contract_v2(authority.clone());
    let asset_addr = sac.address();

    client.init(&authority, &worker, &asset_addr);
    (env, contract_id, asset_addr, authority, worker)
}

/// Fund the payment contract with `amount` asset units.
fn fund(env: &Env, asset_addr: &Address, contract_id: &Address, authority: &Address, amount: i128) {
    let sac = StellarAssetClient::new(env, asset_addr);
    sac.mock_all_auths().mint(contract_id, &amount);
}

#[test]
fn init_sets_authority_worker_asset() {
    let (env, contract_id, asset_addr, authority, worker) = setup();
    let client = SentinelSacClient::new(&env, &contract_id);
    assert_eq!(client.asset(), asset_addr);
...
fn configure_provider_sets_config() {
    let (env, contract_id, _asset, authority, _worker) = setup();
    let client = SentinelSacClient::new(&env, &contract_id);
...
fn free_probe_records_without_payment() {
    let (env, contract_id, _asset, authority, worker) = setup();
    let client = SentinelSacClient::new(&env, &contract_id);
    let payout = Address::generate(&env);
    client.mock_all_auths().configure_provider(
        &String::from_str(&env, "github"),
...
fn paywalled_probe_transfers_and_records() {
    ...
    // Fund the contract with 10 USDC (10_000_000 micro-USDC)
    fund(&env, &asset_addr, &contract_id, &authority, 10_000_000);
